...
def get_api_keys():
    ...
    # Try Streamlit secrets (correct syntax)
    try:
...
def call_groq(prompt, api_key):
    """Call Groq API - UPDATED MODEL NAME"""
    if not api_key:
    ...
    try:
        response = requests.post(
            ...
            headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"},
            json={
                "model": "llama-3.3-70b-versatile",  # FIXED: Updated from 3.1 to 3.3
                "messages": [{"role": "user", "content": prompt}],
                "temperature": 0.3,
                "max_tokens": 1500
            },
...
"""
    for i, a in enumerate(research[:3], 1):
        prompt += f"\n{i}. {a['title']} ({a['year']}): {a['abstract'][:150]}..."
    
    if drug_info:
        prompt += "\n\nMEDICATIONS:\n"
        for d, info in drug_info.items():
            if info.get('found'):
                prompt += f"- {d}: {info.get('indications', [''])[0][:100]}...\n"
    
    if interactions:
        prompt += "\n\nINTERACTIONS:\n"
        for inter in interactions:
            prompt += f"- {inter.get('description', '')}\n"
...
Note: Educational only. Consult healthcare professionals."""
...
def main():
    ...
    ai_list = []
    if keys['groq']: ai_list.append("Groq")
    if keys['openai']: ai_list.append("OpenAI")
    if keys['gemini']: ai_list.append("Gemini")
    
    if ai_list:
        c4.markdown(f'<span class="status-pill status-on">● AI ({", ".join(ai_list)})</span>', unsafe_allow_html=True)
    else:
    ...
    with tab1:
        ...
        query = st.text_area("Your question", placeholder="What are treatment options for Type 2 diabetes?", height=100, label_visibility="collapsed")
        ...
        if st.button("✨ Analyze", use_container_width=False):
            if query:
                ...
                if drug_info: summary += f" • {len(drug_info)} medications"
                if interactions: summary += f" • {len(interactions)} interactions"
                ...
                if has_any_key:
                    ...
                    st.markdown("""
                        <div class="warning-box">
                            ⚠️ <b>Add API keys in Streamlit Secrets:</b><br><br>
                            GROQ_API_KEY = "gsk_..." (Free: console.groq.com/keys)<br>
                            OPENAI_API_KEY = "sk-..." (Paid)<br>
                            GEMINI_API_KEY = "AI..." (Free: aistudio.google.com/apikey)
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    for a in research[:4]:
                        st.markdown(f"""
                            <div class="result-card">
                                ...
                                <p>{a['abstract'][:220]}...</p>
                                <a href="{a['url']}" target="_blank" style="color:#8b5cf6;">View on PubMed →</a>
                            </div>
                        """, unsafe_allow_html=True)
